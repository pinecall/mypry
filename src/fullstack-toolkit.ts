...
export const DEBUGGER_INSTRUCTIONS = [
  ...
  '- debugger_browse auto-detects backend pauses — if a breakpoint fires, the response includes backend state.',
  ...
  '',
  'Remove a breakpoint:',
  '  debugger_breakpoints { remove: "1" }     ← by ID from the list',
  '  debugger_breakpoints { remove: "all" }   ← remove ALL breakpoints',
  '',
  '═══════════════════════════════════════════════════════',
  'COMMON PITFALLS',
  '═══════════════════════════════════════════════════════',
  ...
  '  - Check file path: use basename only ("auth.ts", not "src/auth.ts")',
  '  - Confirm the code path executes (trigger the right endpoint/action)',
  '  - For Next.js: connect to PORT+1, not the main port',
  '',
  '• "No inspector found"?',
  '  - App not started with --inspect. Use debugger_inject instead.',
  '  - Or ask user: node --inspect=9229 app.js',
  '',
  '• Paused but locals are empty?',
  '  - You may be paused in framework code. Call debugger_step { mode: "over" }',
  '  - Or check call_stack in debugger_state — find your code frame',
  '',
  '• Exception BP fires on React/framework errors?',
  '  - This is filtered automatically (justMyCode). If it still fires,',
  '    the exception is in YOUR code, not the framework.',
  '',
  '═══════════════════════════════════════════════════════',
